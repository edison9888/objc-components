...
@interface XMPPXFacebookPlatformAuthentication (Extra)
//{
//    NSString *nonce;
//    NSString *method;
...
+ (NSArray *)permissions {
    return [NSArray arrayWithObjects:   @"user_about_me", 
                                        @"user_birthday", 
                                        @"email", 
                                        @"user_photos", 
                                        @"read_stream", 
...
- (BOOL)supportsXFacebookPlatform
{
	...
	if (state > STATE_XMPP_STARTTLS_1)
	{
...
//		DDLogSend(@"SEND: X-FACEBOOK-PLATFORM");
		...
		{
			XMPPXFacebookPlatformAuthentication *auth = [[XMPPXFacebookPlatformAuthentication alloc] initWithChallenge:response appId:appId accessToken:facebook.accessToken];
...
@implementation XMPPXFacebookPlatformAuthentication (Extra)
...
//	[buffer appendFormat:@"api_key=%@&", FacebookKey];
